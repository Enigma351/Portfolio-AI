...
export default function Window({ title, id, children }) {
  ...
  return (
    <motion.div
      ...
      className={`absolute shadow-[0_20px_50px_rgba(0,0,0,0.4)] rounded-xl border overflow-hidden backdrop-blur-xl transition-all duration-300
    ...
    >
      ...
      <div 
        className="h-10 border-b border-white/5 bg-white/5 flex items-center justify-between px-3 cursor-move rounded-t-xl select-none"
        onPointerDown={(e) => {
          setActive(id);
          dragControls.start(e);
        }} // Ensures grabbing title bar activates window and starts drag
      >
        <div className="flex items-center gap-3 md:gap-2 relative z-50 pointer-events-auto">
          <div 
             onPointerDown={(e) => { e.stopPropagation(); closeApp(id); }} 
             className={`${isMobile ? 'w-8 h-8' : 'w-6 h-6 md:w-4 md:h-4'} flex items-center justify-center cursor-pointer group px-1`}
          >
